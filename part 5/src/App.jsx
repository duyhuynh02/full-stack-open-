...
const App = () => {
    ...
    return (
      <div>
        <div style={hideBlogWhenVisible}>
          <button onClick={() => setBlogCreateVisible(true)}>create new note</button>
        </div>
        <div style={showBlogWhenVisible}>
          <BlogForm createBlog={addBlog}/>
          <button onClick={() => setBlogCreateVisible(false)}>hide new note</button>
        </div>
      </div>
    )
  }
  ...
  const allBlogsUser = () => {
    const allBlogsFromUser = blogs.filter(blog => (blog.user && (blog.user.username === user.username))).sort()
    // console.log('allBlogsFromUser: ', allBlogsFromUser)
    allBlogsFromUser.sort(compareByLike)

    return (
      allBlogsFromUser.map(blog => <Blog key={blog.id} blog={blog} handleLikes={handleLikes}/>)
    )
  }
